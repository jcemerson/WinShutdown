...
def WindowsCheck():
    ...
    if platform.system() == 'Windows' and platform.release() == '10':
        ...
        proc = subprocess.Popen( 'if defined KIVY_GL_BACKEND (echo True) else (echo False)' , shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE )
        out, err = proc.communicate()

        # If environment variable KIVY_GL_BACKEND does NOT exist, then
        if out == False:

            # Create environment variable to set KIVY_GL_BACKEND to angle (instead of glew) to prevent issue detecting OpenGL version
